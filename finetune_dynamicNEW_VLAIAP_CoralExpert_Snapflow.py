"""
Fine-Tune SmolVLA with VLA-IAP, CORAL Experts, LoRA-SP & SnapFlow
===================================================================
Pipeline: finetune_dynamicNEW_VLAIAP_CoralExpert_Snapflow
RTX 4070 SUPER (12GB) | fp16/bf16 | lerobot 0.4.4

Architecture Enhancements (vs. finetune_dynamic_lora_prunning_snapflow):
  1. Hierarchical STAR Router with Curriculum Learning & Diversity-Driven Loss
     - Layer groups: Foundation (L0-L7), Spatial (L8-L11), Action-Refine (L12-L15)
     - Gumbel-Softmax temperature annealing with Action Entropy penalty
     - SnapFlow MSE feedback loop to break saturation-skip
  2. VLA-IAP (Interaction-Aligned Pruning) replaces basic ADP
     - IoU-based Interaction Lock (Conservative ↔ Aggressive mode)
     - Geometric Priors for structural anchor preservation
     - Temporal Smoothing across frames
  3. LoRA-SP (Spectral Rank Adaptation) — dynamic singular value routing
     - U·diag(s(x))·V decomposition with input-dependent s(x)
     - Spectral Concentration Loss for rank sparsity
  4. CORAL Expert Manager — language-routed multi-task LoRA experts
     - Zero-overhead LoRA swapping based on task instruction
     - Gradient isolation across task domains
  5. SnapFlow 1-NFE Self-Distillation with CogKD
     - Cognition Self-Knowledge Distillation to prevent Cognitive Collapse
     - MSE feedback to STAR Router for closed-loop depth control

Training Schedule:
  Phase 1 (Steps 0-5K):     Hierarchical Gating + CORAL Expert routing
  Phase 2 (Steps 5K-15K):   LoRA-SP + VLA-IAP + Diversity Loss activation
  Phase 3 (Steps 15K-20K):  SnapFlow self-distillation + CogKD
"""
...
dyn_cfg = TRAINING["vlaiap_coral_snapflow"]

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
OUTPUT_DIR = Path(f"d:/EyetechCode/results/vlaiap_coral_snapflow")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 70)
print("  SmolVLA Fine-Tune: VLA-IAP + CORAL Experts + LoRA-SP + SnapFlow")
print("=" * 70)
...
# =====================================================================
#  MODULE 2: VLA-IAP (Interaction-Aligned Pruning)
# =====================================================================

class VLAIAPTokenPruner(nn.Module):
    """VLA-IAP: Interaction-Aligned Token Pruning.
    ...
    def forward(self, token_embeddings, token_mask, state,
        ...
        # 2. Determine pruning mode per sample
        ...
        # 3. Compute keep ratio: conservative vs aggressive
        keep_ratio = (1.0 - is_aggressive) * self.conservative_ratio + \
                     is_aggressive * self.aggressive_ratio  # (B, 1)

        # 4. Action granularity override: fine actions → always keep more tokens
        ...
        # 5. Compute token importance scores
        ...
        # 6. Geometric prior anchors — boost importance of structural anchors
        ...
        # 7. Temporal smoothing: blend with previous frame's importance
        ...
        # 8. Select top-K tokens based on importance
...
class DynamicVLAIAPCoralWrapper(nn.Module):
    ...
    def compute_context_features(self, state, images_emb=None, actions=None):
        """Compute extended context features for Hierarchical Router.
        ...
        if actions is not None:
            action_entropy = self.star_router.compute_action_entropy(delta_s)
        else:
            action_entropy = torch.zeros(B, 1, device=device)
    ...
    def forward_with_skip(self, batch, tau=1.0, global_step=0,
        ...
        e_view, delta_s_norm, accel_norm, action_entropy = \
            self.compute_context_features(state, all_img_emb, actions)

        # 4. VLA-IAP: Interaction-Aligned Token Pruning
...
# =====================================================================
print("\n" + "=" * 70)
print("  PHASE 2: Load SmolVLA Base + Inject VLA-IAP / CORAL / LoRA-SP")
print("=" * 70)
...
print("\n  Creating DynamicVLAIAPCoralWrapper...")
dyn_wrapper = DynamicVLAIAPCoralWrapper(smolvla, dyn_cfg).to(DEVICE)
...
print(f"  VLA-IAP Pruner params: {sum(p.numel() for p in dyn_wrapper.token_pruner.parameters())/1e6:.4f}M")
...
# =====================================================================
#  PHASE 3: Training Phase 1 — Hierarchical Gating + CORAL Expert
# =====================================================================
print("\n" + "=" * 70)
print("  PHASE 3: Training Phase 1 — Hierarchical Gating + CORAL Expert")
print("=" * 70)
...
for step in pbar:
    ...
    # Curriculum: anneal lambda_cost from very low to moderate
    lambda_cost_curr = dyn_cfg["lambda_cost"] * 0.1 * (1 + 9 * progress)
...
# =====================================================================
#  PHASE 4: Training Phase 2 — LoRA-SP + VLA-IAP + Diversity
# =====================================================================
print("\n" + "=" * 70)
print("  PHASE 4: Training Phase 2 — LoRA-SP + VLA-IAP + Diversity Loss")
print("=" * 70)
...
print(f"  VLA-IAP: conservative={dyn_cfg['iap_conservative_ratio']}, aggressive={dyn_cfg['iap_aggressive_ratio']}")
print(f"  IoU threshold: {dyn_cfg['iap_iou_threshold']}, temporal momentum: {dyn_cfg['iap_temporal_momentum']}")

pbar = tqdm(range(phase2_steps), desc="  Phase2 LoRA+IAP", ncols=110)
for step in pbar:
    opt_p2.zero_grad()
    accum_loss = 0
    accum_dict = {}
...
ax.set_xlabel('Step'); ax.set_ylabel('Loss')
ax.set_title('Phase 2: LoRA-SP + VLA-IAP'); ax.grid(True, alpha=0.3)
...
plt.suptitle('VLA-IAP + CORAL + LoRA-SP + SnapFlow + CogKD — Training Curves', fontsize=14, fontweight='bold')
plt.tight_layout()
...
print("\n  Evaluating VLA-IAP + CORAL + SnapFlow Model...")
...
ax.bar(x, dyn_results['mse_per_episode'], 0.6, label='VLA-IAP+CORAL', color=blue)
...
ax.bar(x, dyn_results['mse_per_joint'], 0.6, label='VLA-IAP+CORAL', color=blue)
...
for ep_idx in eval_ep_list[:3]:
    ...
    for t in range(n):
        ...
        cv2.putText(frame, f'Ep {ep_idx} | Frame {t}/{n} | VLA-IAP+CORAL+Snap', (340, 22),
        ...
        cv2.putText(frame, 'VLA-IAP+CORAL SmolVLA', (55, yb), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,160,0), 1)
...
report = {
    ...
    'results': {
        'label': 'SmolVLA + VLA-IAP + CORAL + LoRA-SP + SnapFlow + CogKD',
    ...
    'techniques': {
        ...
        'VLA-IAP': f'Interaction-Aligned Pruning (IoU threshold={dyn_cfg["iap_iou_threshold"]}, conservative={dyn_cfg["iap_conservative_ratio"]}, aggressive={dyn_cfg["iap_aggressive_ratio"]})',
...
# =====================================================================
print("\n" + "=" * 70)
print("  PIPELINE COMPLETE: finetune_dynamicNEW_VLAIAP_CoralExpert_Snapflow")
print("=" * 70)
